...
export class UpdatePartUseCase {
  ...
  async execute(input: UpdatePartInput): Promise<Part> {
    ...
    const currentPart = await this.partRepository.findById(input.id);
    ...
    const partWithSameCode = await this.partRepository.findByCode(
      nomenclature.code
    );

    if (partWithSameCode && partWithSameCode.id !== input.id) {
    ...
    return this.partRepository.update(input.id, {
